      ...
      {cartItems.map((item) => (
        <tr key={item.id} className="border-b border-[#e0cdb8]">
         ...
         <div className="flex gap-1  sm:flex-row flex-col bg-red-00 w-[30px] sm:h-[30px] h-[90px]">
               <button onClick={() => addToCart(item)} className="bg-green-600  hover:bg-green-700 text-white px-2 cursor-pointer py-1 rounded">+</button>
            <button onClick={() => decreaseQuantity(item.id)} className="bg-yellow-500 hover:bg-yellow-600 cursor-pointer text-white px-2 py-1 rounded">-</button>
            <button onClick={() => removeFromCart(item.id)} className="bg-red-600 hover:bg-red-700 cursor-pointer text-white px-2 py-1 rounded">x</button>
         </div>
          </td>
        </tr>
      ))}
